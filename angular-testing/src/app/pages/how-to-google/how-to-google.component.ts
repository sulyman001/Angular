...
@Component({
  selector: 'app-how-to-google',
  templateUrl: './how-to-google.component.html',
  styleUrls: ['./how-to-google.component.scss']
})
export class HowToGoogleComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.titleService.setTitle(`How To Sell Google Play Gift Cards For Cash`);
    this.metaService.updateTag({ name: 'description', content: `Learn how to sell Google Play Gift Card in Nigeria and Ghana at the highest rates and where to sell Google Play Gift Cards for cash and get paid Instantly.` });
  ...
  selectCategory() {
    this.filteredSubcategories = this.subcategories.filter((item) => item.category?.name?.toLowerCase().includes('google'));
    this.payOut = 0;
  }
