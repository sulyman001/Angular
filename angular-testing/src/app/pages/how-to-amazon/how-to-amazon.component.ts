...
@Component({
  selector: 'app-how-to-amazon',
  templateUrl: './how-to-amazon.component.html',
  styleUrls: ['./how-to-amazon.component.scss']
})
export class HowToAmazonComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.titleService.setTitle(`How To Sell Amazon Gift Cards For Cash`);
    this.metaService.updateTag({ name: 'description', content: `Learn how to sell Amazon Gift Card in Nigeria and Ghana at the highest rates and where to sell Amazon Gift Cards for cash and get paid Instantly.` });
  ...
  selectCategory() {
    this.filteredSubcategories = this.subcategories.filter((item) => item.category?.name?.toLowerCase().includes('amazon'));
    this.payOut = 0;
  }
