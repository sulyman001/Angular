...
@Component({
  selector: 'app-how-to-itunes',
  templateUrl: './how-to-itunes.component.html',
  styleUrls: ['./how-to-itunes.component.scss']
})
export class HowToItunesComponent {
  ...
  ngOnInit() {
    ...
    this.titleService.setTitle(`How To Sell Apple iTunes Gift Cards For Cash`);
    this.metaService.updateTag({ name: 'description', content: `Learn how to sell Apple iTunes Gift Card in Nigeria and Ghana at the highest rates and where to sell Apple iTunes Gift Cards for cash and get paid Instantly.` });
  ...
  selectCategory() {
    this.filteredSubcategories = this.subcategories.filter((item) => item.category?.name?.toLowerCase().includes('itunes'));
    this.payOut = 0;
  }
